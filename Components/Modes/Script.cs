...
/* AdminCon 7.0 Command Line Interface Edition - Source Code - Script.cs
* Intro: Read and execute from script
...
namespace AdminCon_CLI_dotnetEdition.Components.Modes
{
    ...
    class Script
    {
        public static ArrayList openScript(String path)
        {
            FileStream fs = new FileStream(path, FileMode.Open, FileAccess.Read);
            StreamReader read = new StreamReader(fs, Encoding.Default);
            String strReadline;
            ArrayList scriptLinesCollection = new ArrayList();
            while ((strReadline = read.ReadLine()) != null)
            {
                scriptLinesCollection.Add(strReadline);
            }
            return scriptLinesCollection;
        }
    }
    /// <summary>
    /// Execute the script line-by-line
    /// </summary>
    class ScriptMode : Script
    {
        ...
        #endregion
        public void ShellExecute(String scriptName)
        {
            Char[] pathAndFilename = scriptName.ToCharArray();
            Int32 splitIndex = scriptName.LastIndexOf("\\");
            List<Char> charList = new List<Char>();
            for(Int32 i = splitIndex+1; i < pathAndFilename.Length; i++)
            {
                charList.Add(pathAndFilename[i]);
            }
            String filename = new String(charList.ToArray());
            Console.Title = filename;
            Console.Write("\n\nac:script/cli>");
            Console.WriteLine();
            foreach (String cmd in openScript(scriptName))
            {
                System.GC.Collect(); //Try to collect garbage once per statement execution.
                command = grammarAnalyzer.fix(cmd);
                ...
                switch (cmdUpper)
                {
                    ...
                    default:
                        ...
                        try
                        {
                            if (toolreg.CheckIfRegistered(command, 1) == true)
                            {
                                //Launch personalized executable
                                System.Diagnostics.Process.Start(ACInfo.locationPath + "\\tools\\" + toolreg.GetProgramFileName(command, 1));
                            }
                            else if (cmds == "HEX")
                            {
                                if (field == null)
                            ...
                            else if (cmds == "KILL")
                            {
                                ...
                                {
                                    if (processRef.CheckIfRegistered(arg, 1))
                                    {
                                        processAlgorithms.KillProcess(processRef.GetProcessName(arg, 1));
                                    }
                                    else
                                    {
                                        try
